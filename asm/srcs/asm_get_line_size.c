...
/*   Updated: 2017/03/12 16:17:52 by mverdier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		asm_check_name_comment_op(t_asm *m_asm, char **split, int n,
		t_op **op_tab)
{
		...
		unsigned int big_size, t_asm *m_asm)
{
	...
	if ((ret = asm_go_to_instruct_size(split, &n, labels, big_size)) < 1)
		return (ret);
	if (!asm_check_name_comment_op(m_asm, split, n, &op_tab))
		return (-1);
